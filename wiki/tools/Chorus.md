...
The atlas already lists AlphaGenome, ChromBPNet (variant-scorer), and BRAIN-MAGNET as standalone tools. Chorus is the first wrapper that makes head-to-head comparison straightforward. For a the proband-WGS noncoding variant of interest, you can ask AlphaGenome, ChromBPNet, and BRAIN-MAGNET about the same site and see whether they agree. Disagreement isn't necessarily a problem -- the models are trained on different cell types and different output modalities -- but it is a useful sanity check.
...
* For the current the proband-WGS noncoding variant shortlist, does AlphaGenome agree with ChromBPNet on direction and magnitude?
* What's the agreement rate between BRAIN-MAGNET (NSC-trained) and Borzoi (RNA-seq-trained) on the autism GWAS noncoding loci?
* Does a percentile-rank cutoff (top 1%, top 5%) materially change which the proband variants stay on the candidate list?
