...
The published model is trained on neural stem cells, not differentiated cortical neurons or fetal cortical organoids -- whether NSC predictions transfer to the cell types most relevant to autism is an open question worth checking with the authors or via independent validation. Also unclear whether the pre-scored SNP table covers GRCh38 variants the proband's WGS uses or requires a liftover.
